...
def send_simple_action_notification(sender, target_org, title,
    ...
    if target_type_key:
        ActionNotification.objects.create(sender=sender,
...
def send_action_notification_to_project_contribs(
        project):  # sent when a new comment is added to a Project
    ...
    for attr in ['communities', 'institutions', 'researchers']:
        ...
        for recipient in recipients:
            send_simple_action_notification(None, recipient, title, 'Projects',
                                            project.unique_id)
...
def send_account_member_invite(
    ...
    UserNotification.objects.create(from_user=invite.sender,
...
def send_user_notification_member_invite_accept(
    ...
    UserNotification.objects.create(to_user=receiver_,
                                    from_user=sender_,
                                    title=title,
                                    message=message,
    ...
    UserNotification.objects.create(to_user=sender_,
                                    from_user=receiver_,
                                    title=title2,
                                    message=message2,
                                    notification_type="Accept",
    ...
    HubActivity.objects.create(action_user_id=member_invite.receiver.id,
