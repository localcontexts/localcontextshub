...
class TestFeatures(TransactionTestCase):
    def setUp(self):
        self.client = Client()
        self.subscribed_researcher = ResearcherFactory(user=UserFactory(), is_submitted=True, is_subscribed=True)
        self.subscribed_researcher_project = ProjectFactory(project_creator=self.subscribed_researcher.user)
        self.unsubscribed_researcher = ResearcherFactory(user=UserFactory(), is_submitted=False , is_subscribed=False)
