...
class UserNotificationFactory(factory.django.DjangoModelFactory):
    ...
    notification_type = factory.fuzzy.FuzzyChoice(choices=[choice[0] for choice in UserNotification.TYPES])
    community = factory.SubFactory(CommunityFactory)
    institution = factory.SubFactory(InstitutionFactory)
    role = factory.fuzzy.FuzzyChoice(choices=[choice[0] for choice in UserNotification.ROLES])
...
class ActionNotificationFactory(factory.django.DjangoModelFactory):
    ...
    title = factory.LazyAttribute(lambda n: faker.sentence()[:200])
    notification_type = factory.fuzzy.FuzzyChoice(choices=[choice[0] for choice in ActionNotification.TYPES])
