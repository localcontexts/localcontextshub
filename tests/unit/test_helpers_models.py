...
from helpers.models import NoticeTranslation
...
class TestNotice(TestCase):
    ...
    def test_notice_set_translation(self):
        ...
        translation = NoticeTranslation.objects.filter(
            notice=new_notice, language_tag=language_tag).first()
...
class TestNoticeTranslation(TestCase):
    ...
    def test_notice_translation_str_method(self):
        ...
        self.assertEqual(
            string,
            f"{new_notice_translation.notice} - {new_notice_translation.language}"
        )
...
class TestEntitiesNotified(TestCase):
    ...
    def test_is_user_in_notified_account(self):
        ...
        assert new_entities_notified.is_user_in_notified_account(
            community_user) is True
        assert new_entities_notified.is_user_in_notified_account(
            institution_user) is True
...
class TestProjectComment(TestCase):
    ...
    def test_new_label_translation_str_method(self):
        ...
        assert isinstance(string, str)
        assert string == 'Comment {} by {}'.format(
            new_project_comment.message, new_project_comment.community)
