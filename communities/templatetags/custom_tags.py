...
def get_label_count(community):
    ...
    for label in BCLabel.objects.prefetch_related('project_bclabels').filter(
            community=community):
        count = count + label.project_bclabels.count()
    for label in TKLabel.objects.prefetch_related('project_tklabels').filter(
            community=community):
...
def connections_count(community):
    contributor_ids = list(
        chain(
            community.contributing_communities.exclude(
                institutions__id=None).values_list('institutions__id',
                                                   flat=True),
            community.contributing_communities.exclude(
                researchers__id=None).values_list('researchers__id',
                                                  flat=True),
        ))
