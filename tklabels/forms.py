...
class CustomizeTKLabelForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            forms.Textarea(
                attrs={
                    'class': 'w-100 margin-top-1 margin-bottom-2',
                    'id': 'label-template-text',
            ...
            forms.ClearableFileInput(
                attrs={
                    'class': 'w-100 hide',
                    'id': 'originalLabelAudioUploadBtn',
                    'onchange': 'showAudioFileName()'
                }),
        }
        ...
        if audiofile:
            ...
            if file_ext not in allowed_extensions:
                raise ValidationError(
                    'Invalid audio file extension. Only MP3, WAV, FLAC, WMA, M4A, etc. files are allowed.'
                )

            allowed_mime_types = [
                'audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/aac',
                'audio/flac', 'audio/x-ms-wma', 'audio/mp4', 'audio/opus',
                'audio/aiff'
            ]
            if audiofile.content_type not in allowed_mime_types:
                raise ValidationError(
                    'Invalid audio file type. Only MP3, WAV, FLAC, WMA, M4A, etc. files are allowed.'
                )
        return audiofile
