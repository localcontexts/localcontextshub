            ...
            if result['success'] and result.get(
                ...
                if User.objects.filter(email=user.email).exists():
                    ...
                    return redirect('register')
                        ...
                        username__iexact=user.username.lower()).exists():
                    messages.error(request,
                    ...
                    HubActivity.objects.create(action_user_id=user.id,
    ...
    def get(self, request, uidb64, token):
        ...
        if user is not None and generate_token.check_token(user, token):
            ...
            messages.add_message(
                ...
                'Profile activation successful. You may now log in.')
    ...
    if request.method == 'POST':
        if form.is_valid():
            active_users = User._default_manager.filter(
                **{
                    '%s__iexact' % User.get_email_field_name():
...
class CustomSocialConnectionsView(ConnectionsView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        social_account = SocialAccount.objects.filter(
            provider=provider, user=request.user).first()
        ...
        if social_account and has_password:
            ...
            messages.error(request,
                           'Please set password first to unlink an account')
            return redirect('link-account')
        return super().dispatch(request, *args, **kwargs)
...
def dashboard(request):
    ...
    affiliation = user.user_affiliations.prefetch_related(
        'communities', 'institutions', 'communities__admins',
        'communities__editors', 'communities__viewers', 'institutions__admins',
        'institutions__editors', 'institutions__viewers').all().first()
...
def manage_organizations(request):
    ...
    affiliations = UserAffiliation.objects.prefetch_related(
        ...
        'institutions__institution_creator').get(user=request.user)
...
def link_account(request):
    ...
    return render(request, 'accounts/link-account.html', {
    ...
    if request.method == "POST":
        if invite_form.is_valid():
            ...
            if User.objects.filter(email=data.email).exists():
                messages.add_message(request, messages.ERROR,
                                     'This user is already in the Hub')
...
def registry(request, filtertype=None):
    try:
        c = Community.approved.select_related(
            'community_creator').prefetch_related(
                'admins', 'editors',
                'viewers').all().order_by('community_name')
        i = Institution.approved.select_related(
            'institution_creator').prefetch_related(
                'admins', 'editors',
                'viewers').all().order_by('institution_name')
        ...
        if ('q' in request.GET) and (filtertype != None):
            ...
            if filtertype == 'communities':
                ...
                researchers_with_otc = r.filter(
                    otc_researcher_url__isnull=False).distinct()
                institutions_with_otc = i.filter(
                    otc_institution_url__isnull=False).distinct()
                cards = return_registry_accounts(None, researchers_with_otc,
            ...
            if request.method == 'POST':
                if 'updatebtn' in request.POST:
                    if 'unsubscribe' in request.POST:
                        ...
                        add_to_mailing_list(str(email), str(name),
                                            str(variables))
                        messages.add_message(
