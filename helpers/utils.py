...
def check_member_role(user, organization):
    ...
    if isinstance(organization,
                  Community) and user == organization.community_creator:
        return 'admin'
    if isinstance(organization,
                  Institution) and user == organization.institution_creator:
        return 'admin'
...
def accept_member_invite(request, invite_id):
    ...
    send_user_notification_member_invite_accept(
        invite)  # Send UserNotifications
    ...
    UserNotification.objects.filter(to_user=invite.receiver,
                                    from_user=invite.sender,
                                    reference_id=invite.id).delete()
...
def accepted_join_request(request, org, join_request_id, selected_role):
    ...
    if JoinRequest.objects.filter(id=join_request_id).exists():
        ...
        if selected_role is None:
            ...
            affiliation = UserAffiliation.objects.get(
                user=join_request.user_from)
            if isinstance(org, Community):
                ...
                if ActionNotification.objects.filter(
                        ...
                        community=org,
                        reference_id=join_request.id).exists():
            ...
            UserNotification.objects.create(to_user=sender,
                                            title=title,
...
def crud_notices(request, selected_notices, selected_translations,
                 organization, project, existing_notices):
    ...
    def create(notice_type):
        if isinstance(organization, (Institution, Researcher)):
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{new_notice.name} was applied to the Project by {name}')
        ...
        for ntype, lang_tag in selected_notice_types_langs:
            ...
            if notice.notice_type == ntype:
                ...
                if not notice.notice_translations.filter(
                        notice_type=ntype, language_tag=lang_tag).exists():
        ...
        for notice in existing_notices:
            ...
            if not notice.notice_type in selected_notices:  # if existing notice not in selected notices, delete notice
                ...
                ProjectActivity.objects.create(
                    project=project,
                    activity=
                    f'{notice.name} was removed from the Project by {name}')
    ...
    if project.bc_labels.filter(community=community).exists():
        ...
        for bclabel in project.bc_labels.filter(community=community).exclude(
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{bclabel.name} Label was removed by {user} | {community.community_name}'
            )
    ...
    if project.tk_labels.filter(community=community).exists():
        for tklabel in project.tk_labels.filter(community=community).exclude(
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{tklabel.name} Label was removed by {user} | {community.community_name}'
            )
    ...
    for bclabel in bclabels:
        if not bclabel in project.bc_labels.all(
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{bclabel.name} Label was applied by {user} | {community.community_name}'
            )
    ...
    for tklabel in tklabels:
        if not tklabel in project.tk_labels.all():
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{tklabel.name} Label was applied by {user} | {community.community_name}'
            )
...
def handle_label_versions(label):
    ...
    if isinstance(label, BCLabel):
        ...
        latest_version = LabelVersion.objects.filter(
            bclabel=label).order_by('-version').first()
        ...
        version = LabelVersion.objects.create(bclabel=label,
                                              version=version_num,
    ...
    if isinstance(label, TKLabel):
        ...
        # If approved version exists, set version number to 1 more than the latest
        latest_version = LabelVersion.objects.filter(
            tklabel=label).order_by('-version').first()
        ...
        version = LabelVersion.objects.create(tklabel=label,
                                              version=version_num,
                                              created_by=label.created_by,
                                              is_approved=True,
                                              approved_by=label.approved_by,
