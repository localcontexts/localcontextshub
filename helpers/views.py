...
def delete_member_invite(request, pk):
    ...
    if UserNotification.objects.filter(to_user=invite.receiver,
                                       from_user=invite.sender,
                                       notification_type='invitation',
                                       reference_id=pk).exists():
...
def download_open_collaborate_notice(request,
    ...
    if dev_prod_or_local(
            request.get_host()) == 'SANDBOX' or not has_permission:
        return redirect('restricted')
    else:
        if researcher_id:
            researcher = get_object_or_404(Researcher, id=researcher_id)
            NoticeDownloadTracker.objects.create(
                researcher=researcher,
                user=request.user,
                open_to_collaborate_notice=True)
...
def download_community_support_letter(request):
    try:
        ...
        if response.status_code == 200:
            ...
            response[
                'Content-Disposition'] = 'attachment; filename="LC_Community_Support_Letter_Template.docx"'
            return response
    except:
        raise Http404()
...
def download_institution_support_letter(request):
    try:
        ...
        if response.status_code == 200:
            ...
            response[
                'Content-Disposition'] = 'attachment; filename="LC_Institution_Support_Letter_Template.docx"'
            return response
    except:
        raise Http404()
...
def boundary_view(request):
    try:
        ...
        if boundary:
            boundary = [
                json.loads(boundary.replace('(', '[').replace(')', ']'))
            ]
        else:
            boundary = []
