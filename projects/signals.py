...
def delete_action_notifications(sender, instance, *args, **kwargs):
    if ActionNotification.objects.filter(
            reference_id=instance.unique_id).exists():
        for notification in ActionNotification.objects.filter(
                reference_id=instance.unique_id):
            notification.delete()
...
def delete_archived_instances(sender, instance, *args, **kwargs):
    if ProjectArchived.objects.filter(
            project_uuid=instance.unique_id).exists():
        for notification in ProjectArchived.objects.filter(
                project_uuid=instance.unique_id):
            notification.delete()
...
def delete_source_project_uuid(sender, instance, *args, **kwargs):
    source_projects = Project.objects.filter(
        source_project_uuid=instance.unique_id)
