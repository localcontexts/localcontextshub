...
def source_project_title(uuid):
    try:
        title = Project.objects.filter(unique_id=uuid).values_list(
            'title', flat=True).first()
        return title
...
def get_all_researchers(researcher_to_exclude):
    if researcher_to_exclude:
        return Researcher.objects.select_related('user').exclude(
            id=researcher_to_exclude.id)
    else:
...
def connections_collaborative_projects(target_account, collaborating_account):
    ...
    if isinstance(target_account, Institution):
        if isinstance(collaborating_account, Institution):
            projects_list = ProjectContributors.objects.filter(
                institutions=collaborating_account).filter(
                    institutions=target_account).values_list(
                        'project__unique_id', flat=True)
        if isinstance(collaborating_account, Community):
            projects_list = target_account.contributing_institutions.filter(
                communities=collaborating_account).values_list(
                    'project__unique_id', flat=True)
        if isinstance(collaborating_account, Researcher):
            projects_list = target_account.contributing_institutions.filter(
                researchers=collaborating_account).values_list(
                    'project__unique_id', flat=True)

    if isinstance(target_account, Community):
        if isinstance(collaborating_account, Community):
            projects_list = ProjectContributors.objects.filter(
                communities=collaborating_account).filter(
                    communities=target_account).values_list(
                        'project__unique_id', flat=True)
        if isinstance(collaborating_account, Institution):
            projects_list = target_account.contributing_communities.filter(
                institutions=collaborating_account).values_list(
                    'project__unique_id', flat=True)
        if isinstance(collaborating_account, Researcher):
            projects_list = target_account.contributing_communities.filter(
                researchers=collaborating_account).values_list(
    ...
    if isinstance(target_account, Researcher):
        if isinstance(collaborating_account, Researcher):
            projects_list = ProjectContributors.objects.filter(
                researchers=collaborating_account).filter(
                    researchers=target_account).values_list(
                        'project__unique_id', flat=True)
        if isinstance(collaborating_account, Community):
            projects_list = target_account.contributing_researchers.filter(
                communities=collaborating_account).values_list(
                    'project__unique_id', flat=True)
        if isinstance(collaborating_account, Institution):
            projects_list = target_account.contributing_researchers.filter(
    ...
    projects = Project.objects.select_related(
        'project_creator').prefetch_related('bc_labels', 'tk_labels').filter(
            unique_id__in=projects_list).order_by('-date_added')
    return projects
