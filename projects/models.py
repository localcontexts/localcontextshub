...
class Project(models.Model):
    ...
    project_privacy = models.CharField(max_length=20,
                                       null=True,
    ...
    project_data_guid = models.CharField(max_length=200,
                                         blank=True,
                                         null=True,
                                         verbose_name="Project data GUID")
    providers_id = models.CharField(max_length=200,
                                    blank=True,
                                    null=True,
                                    verbose_name="Provider's ID")
    project_boundary_geojson = models.JSONField(
        blank=True, null=True, verbose_name="Project boundary GeoJSON")
    urls = models.JSONField(blank=True, null=True, verbose_name="URLs")
    date_added = models.DateTimeField(auto_now_add=True, null=True)
    date_modified = models.DateTimeField(auto_now=True, null=True)
    source_project_uuid = models.UUIDField(null=True,
                                           verbose_name="Source Project UUID",
                                           blank=True,
                                           db_index=True)
    related_projects = models.ManyToManyField("self",
                                              ...
                                              db_index=True)
    bc_labels = models.ManyToManyField("bclabels.BCLabel",
                                       verbose_name="BC Labels",
                                       blank=True,
                                       related_name="project_bclabels",
                                       db_index=True)
    tk_labels = models.ManyToManyField("tklabels.TKLabel",
                                       verbose_name="TK Labels",
                                       blank=True,
                                       related_name="project_tklabels",
                                       db_index=True)
...
class ProjectContributors(models.Model):
    ...
    institutions = models.ManyToManyField(
        Institution, blank=True, related_name="contributing_institutions")
    communities = models.ManyToManyField(
        Community, blank=True, related_name="contributing_communities")
    researchers = models.ManyToManyField(
        Researcher, blank=True, related_name="contributing_researchers")
...
class ProjectPerson(models.Model):
    project = models.ForeignKey(Project,
                                null=True,
                                on_delete=models.CASCADE,
                                related_name="additional_contributors")
...
class ProjectCreator(models.Model):
    community = models.ForeignKey(Community,
                                  on_delete=models.CASCADE,
                                  related_name='community_created_project',
                                  null=True,
                                  blank=True)
    institution = models.ForeignKey(Institution,
                                    on_delete=models.CASCADE,
                                    related_name='institution_created_project',
                                    null=True,
                                    blank=True)
    researcher = models.ForeignKey(Researcher,
                                   on_delete=models.CASCADE,
                                   related_name='researcher_created_project',
                                   null=True,
                                   blank=True)
    project = models.ForeignKey(Project,
                                on_delete=models.CASCADE,
    ...
    def is_user_in_creator_account(self, user, is_created_by):
        ...
        if is_created_by['community'] and self.community.is_user_in_community(
                user
        ):  # is user a member of the community that created the project
            is_user_in_account = True
        elif is_created_by[
                'institution'] and self.institution.is_user_in_institution(
                    user
                ):  # is user a member of the institution tha created the project
            is_user_in_account = True
        elif is_created_by['researcher'] and Researcher.objects.filter(
                user=user
        ).exists() and self.researcher == Researcher.objects.get(
                user=user
        ):  # does this user have a researcher account and is it the same researcher account which created the project
            is_user_in_account = True
        else:
            is_user_in_account = False
    ...
    class Meta:
        indexes = [
            models.Index(
                fields=['project', 'community', 'institution', 'researcher'])
        ]
...
class ProjectNote(models.Model):
    ...
    community = models.ForeignKey(Community,
                                  on_delete=models.CASCADE,
                                  related_name='community_note',
                                  null=True,
                                  blank=True)
    sender = models.ForeignKey(User,
                               on_delete=models.CASCADE,
                               null=True,
                               related_name="project_note_sender",
                               blank=True)
