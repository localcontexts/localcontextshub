    ...
    for status in statuses:
        ...
        if project_status == 'seen':
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{name} from {community.community_name} set the Project status to Seen'
            )
        ...
        if project_status == 'pending':
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{name} from {community.community_name} set the Project status to Labels Pending'
            )
        ...
        if project_status == 'not_pending':
            ...
            ProjectActivity.objects.create(
                project=project,
                activity=
                f'{name} from {community.community_name} set the Project status to No Labels Pending'
            )
        ...
        if creator.institution:
            send_simple_action_notification(user, creator.institution, title,
                                            'Projects', reference_id)
        if creator.researcher:
            send_simple_action_notification(user, creator.researcher, title,
                                            'Projects', reference_id)


def project_status_seen_at_comment(user, community, creator, project):
    if ProjectStatus.objects.filter(project=project,
        ...
        if status.seen == False:
            ...
            # Send Notification
            if creator.institution:
                send_simple_action_notification(user, creator.institution,
                                                title, 'Projects',
                                                reference_id)
            if creator.researcher:
                send_simple_action_notification(user, creator.researcher,
                                                title, 'Projects',
                                                reference_id)


# CONTRIBUTORS
def process_contributors(request, project, contributors, current_ids,
                         incoming_ids, model):
    ...
    for item in model.objects.filter(id__in=matching_ids):
        send_simple_action_notification(None, item,
                                        'Edits have been made to a Project',
                                        'Projects', project.unique_id)
    ...
    for item in model.objects.filter(id__in=unmatching_ids):
        ...
        send_simple_action_notification(
            None, item,
            f'Your {account} has been added as a contributor on a Project',
            'Projects', project.unique_id)
...
def add_to_contributors(request, account, project):
    ...
    institutions_list = [
        eval(i) for i in request.POST.getlist('selected_institutions')
    ]
    researchers_list = [
        eval(i) for i in request.POST.getlist('selected_researchers')
    ]
    communities_list = [
        eval(i) for i in request.POST.getlist('selected_communities')
    ]
    ...
    if '/edit-project/' in request.path:
        current_institution_ids = list(
            contributors.institutions.all().values_list(
                'id', flat=True))  # current list of institutions
        current_researcher_ids = list(
            contributors.researchers.all().values_list(
                'id', flat=True))  # current list of researchers
        current_community_ids = list(
            contributors.communities.all().values_list(
                'id', flat=True))  # current list of communities

        if isinstance(account, Institution):
            if account.id in current_institution_ids:
                current_institution_ids.remove(
                    account.id
                )  # Project creator account is removed from list of possible institutions
            ...
            if account.id in current_researcher_ids:
                current_researcher_ids.remove(
                    account.id
                )  # Project creator account is removed from list of possible researchers
            ...
            if account.id in current_community_ids:
                current_community_ids.remove(
                    account.id
                )  # Project creator account is removed from list of possible communities
            if account.id in communities_list:
                communities_list.remove(account.id)

        process_contributors(request, project, contributors.institutions,
                             current_institution_ids, institutions_list,
                             Institution)
        process_contributors(request, project, contributors.researchers,
                             current_researcher_ids, researchers_list,
                             Researcher)
        process_contributors(request, project, contributors.communities,
                             current_community_ids, communities_list,
                             Community)
        ...
        for institution in Institution.objects.filter(
                id__in=institutions_list):
            contributors.institutions.add(
                institution)  # add selected contribs to contribs
            send_simple_action_notification(
                None, institution,
                'Your institution has been added as a contributor on a Project',
                'Projects', project.unique_id)
        ...
        for researcher in Researcher.objects.filter(id__in=researchers_list):
            contributors.researchers.add(
                researcher)  # add selected contribs to contribs
            send_simple_action_notification(
                None, researcher,
                'You have been added as a contributor on a Project',
                'Projects', project.unique_id)
        ...
        for community in Community.objects.filter(id__in=communities_list):
            contributors.communities.add(
                community)  # add selected contribs to contribs
            send_simple_action_notification(
                None, community,
                'Your community has been as a contributor on a Project',
                'Projects', project.unique_id)
...
def return_project_search_results(request, queryset):
    q = request.GET.get('q')
    if q:
        vector = SearchVector('title', 'description', 'unique_id',
                              'providers_id')
        query = SearchQuery(q)
        results = queryset.annotate(rank=SearchRank(vector, query)).filter(
            rank__gte=0.001).order_by('-rank')  # project.rank returns a num
    else:
        results = None
    return results
...
def return_project_labels_by_community(project):
    bc_labels = BCLabel.objects.filter(
        project_bclabels=project).select_related('community').order_by(
            'community__community_name', 'name')
    tk_labels = TKLabel.objects.filter(
        project_tklabels=project).select_related('community').order_by(
            'community__community_name', 'name')

    label_groups = {}
    for community, group in itertools.groupby(bc_labels,
                                              key=lambda x: x.community):
        bc_labels_list = list(group)
        label_groups.setdefault(
            community, {
                'bc_labels': bc_labels_list,
                'tk_labels': [],
                'bc_label_count': len(bc_labels_list),
                'tk_label_count': 0
            })

    for community, group in itertools.groupby(tk_labels,
                                              key=lambda x: x.community):
        ...
        if community in label_groups:
            ...
            label_groups[community] = {
                'bc_labels': [],
                'tk_labels': tk_labels_list,
                'bc_label_count': 0,
                'tk_label_count': len(tk_labels_list)
            }
