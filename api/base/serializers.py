...
class LabelTranslationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('translated_name', 'language_tag', 'language',
                  'translated_text')


class BCLabelSerializer(serializers.ModelSerializer):
    community = SerializerMethodField()
    translations = LabelTranslationSerializer(source="bclabel_translation",
                                              many=True)

    class Meta:
        model = BCLabel
        fields = ('unique_id', 'name', 'label_type', 'language_tag',
                  'language', 'label_text', 'img_url', 'svg_url', 'audiofile',
                  'community', 'translations', 'created', 'updated')
...
class TKLabelSerializer(serializers.ModelSerializer):
    community = SerializerMethodField()
    translations = LabelTranslationSerializer(source="tklabel_translation",
                                              many=True)
...
class NoticeSerializer(serializers.ModelSerializer):
    translations = NoticeTranslationsSerializer(source="notice_translations",
                                                many=True)
...
class ProjectSerializer(serializers.ModelSerializer):
    created_by = ProjectCreatorSerializer(source="project_creator_project",
                                          many=True)
    ...
    class Meta:
        model = Project
        fields = ('unique_id', 'providers_id', 'source_project_uuid',
                  'project_page', 'title', 'project_privacy', 'date_added',
                  'date_modified', 'created_by', 'notice', 'sub_projects',
                  'related_projects', 'project_boundary_geojson')
    ...
    def get_sub_projects(self, obj):
        return [
            p.unique_id
            for p in Project.objects.filter(source_project_uuid=obj.unique_id)
        ]
...
class ProjectNoNoticeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('unique_id', 'providers_id', 'source_project_uuid',
                  'project_page', 'title', 'project_privacy', 'date_added',
                  'date_modified', 'created_by', 'bc_labels', 'tk_labels',
                  'sub_projects', 'related_projects',
