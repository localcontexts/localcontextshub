...
if (window.location.href.includes("sandbox.localcontextshub")) {
  ...
  if (regHeader) {
    regHeader.style.marginTop = "50px";
  }
...
if (
  window.location.href.includes("create-community") ||
  window.location.href.includes("create-institution") ||
  window.location.href.includes("connect-researcher")
) {
...
function fetchLanguages() {
  const endpoint =
    "https://raw.githubusercontent.com/biocodellc/localcontexts_json/main/data/ianaObj.json";
...
function autocomplete(inp, arr) {
  ...
  inp.addEventListener("input", function (e) {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (
        removeAccents(arr[i]).substr(0, val.length).toUpperCase() ==
        val.toUpperCase()
      ) {
  ...
  function addActive(x) {
    ...
    for (var i = 0; i < x.length; i++) {
      ...
      if (
        unaccentedItemValue
          .toUpperCase()
          .indexOf(unaccentedValue.toUpperCase()) > -1
      ) {
  ...
  function showClearLangBtn(elem) {
    ...
    clearLangBtn.addEventListener("click", function (e) {
      ...
      if (
        langInput.getAttribute("readonly", true) &&
        langInput.classList.contains("readonly-input")
      ) {
...
function translationFormValidation() {
  ...
  const addTranslationForms = document.querySelectorAll(
    ".add-translation-form"
  );
  const currentTranslationForms = document.querySelectorAll(
  ...
  invalidInputs = 0;
  if (
    window.location.href.includes("/labels/customize") &&
    mainLangInput.value != "" &&
    !mainLangInput.classList.contains("readonly-input")
  ) {
    invalidInputs += 1;
  }
  for (var i = 0; i < addTranslationForms.length; i++) {
    var translatedNameInput = addTranslationForms[i].querySelector(
      '[id$="translated_name"]'
    );
    var translatedLangInput =
      addTranslationForms[i].querySelector('[id$="language"]');
    var translatedTextInput = addTranslationForms[i].querySelector(
      '[id$="translated_text"]'
    );

    if (
      (translatedLangInput.value != "" &&
        !translatedLangInput.classList.contains("readonly-input")) ||
  ...
  for (var i = 0; i < currentTranslationForms.length; i++) {
    var translatedNameInput = currentTranslationForms[i].querySelector(
      '[id$="translated_name"]'
    );
    var translatedLangInput =
      currentTranslationForms[i].querySelector('[id$="language"]');
    var translatedTextInput = currentTranslationForms[i].querySelector(
      '[id$="translated_text"]'
    );

    if (
      (translatedNameInput.value != "" && translatedLangInput.value == "") ||
  ...
  if (invalidInputs == 0) {
    ...
    saveLabelBtnValidation("disable");
  }
}
...
function customText(imgDiv) {
  ...
  pDivs.forEach((node) => {
    ...
    if (nodeID.includes(labelID)) {
      node.classList.add("label-name-active");
    } else {
      node.classList.remove("label-name-active");
    }
  });
}
...
function findLabelAndSetValues(
  labels,
  id,
  selectedLanguage,
  label_name,
  ...
  const label = labels.find(
    (label) =>
      id === label.labelCode && selectedLanguage === label.translated_language
  );
  if (label) {
...
function expandBCLabel(img) {
  ...
  txtDivs.forEach((node) => {
    if (node == img.parentElement.nextElementSibling) {
      node.classList.replace("grey-text", "darkteal-text");
    } else {
      node.classList.replace("darkteal-text", "grey-text");
    }
  });
  ...
  let whyUseLabelTextProv = document.getElementById(
    "why-use-this-label-bc-prov"
  );
  ...
  let whyUseLabelTextProt = document.getElementById(
    "why-use-this-label-bc-prot"
  );

  // Permissions
  let infoPerms = document.getElementById("bclabel-info-perm");
  let titlePerms = document.getElementById("bc-label-title-perm");
  let templateTextPerms = document.getElementById(
    "label-template-text-bc-perm"
  );
  let whyUseLabelTextPerms = document.getElementById(
    "why-use-this-label-bc-perm"
  );
...
function checkLabelExists(label, selectedLabelCode, labelType) {
  ...
  if (btnProv || btnProt || btnPerms) {
    ...
    let inputs = Array.from(
      document.querySelectorAll(`.check${labelType}LabelType`)
    );
    let values = [];
...
function displayExpandedImage(labelCode) {
  ...
  for (let i = 0; i < imgArray.length; i++) {
    ...
    if (imgArray[i].id.slice(21) == labelCode) {
      imgArray[i].classList.replace("hide", "show");
    } else {
      imgArray[i].classList.replace("show", "hide");
    }
  }
}
    ...
    for (const mutation of mutationsList) {
      if (
        ...
        if (selectedLanguage == "English" || selectedLanguage == "") {
          language_support.style.display =
            language_support.style.display === "none" ? "none" : "none";
          fetchLabels("both").then(populate);
          ...
          try {
            ...
            if (id.startsWith("b") && data.bcLabels) {
              ...
              console.error(
                `Data for '${
                  id.startsWith("b") ? "bcLabels" : "tkLabels"
          ...
          language_support.style.display =
            language_support.style.display === "none" ? "block" : "block";
          fetchLabels("both").then(populate);
        }
      }
    }
...
function expandCCNotice(noticeDiv) {
  ...
  let allPTags = Array.from(
    document.querySelectorAll(".cc-notice__container p")
  );
...
function expandDisclosureNotice(noticeDiv) {
  ...
  let allDivs = document.querySelectorAll(
    ".disclosure-notice__expanded-container"
  );
...
if (
  window.location.href.includes("/labels/customize") ||
  window.location.href.includes("/labels/edit")
) {
  const addTranslationBtn = document.getElementById("add-translation-btn");
  if (addTranslationBtn) {
    addTranslationBtn.addEventListener("click", cloneForm, false);
  }

  // h/t: https://www.brennantymrak.com/articles/django-dynamic-formsets-javascript
  const initialTargetNodes = document.querySelectorAll(
    '[id^="id_form-"][id$="-language"]'
  );
  ...
  function cloneForm(e) {
    ...
    let newErrorDiv = newForm.querySelector(
      `#id_form-${formNum}-language_support`
    );
    if (newErrorDiv) {
      newErrorDiv.style.display = "none";
    }

    newFormLangButton = newForm.querySelector(
      'button[name="clear-language-btn"]'
    );
    ...
    const updatedTargetNodes = document.querySelectorAll(
      '[id^="id_form-"][id$="-language"]'
    );
    disconnectObserver();
    ...
    for (const mutation of mutationsList) {
      if (
        ...
        if (idMatches) {
          ...
          const translatedNameElement = document.querySelector(
            `#id_form-${formNumber}-translated_name`
          );
          const translationSupport = document.querySelector(
            `#id_form-${formNumber}-language_support`
          );
          const translatedTextElement = document.querySelector(
            `#id_form-${formNumber}-translated_text`
          );
          if (supportedLanguages.includes(required_language)) {
            translationSupport.style.display =
              translationSupport.style.display === "none" ? "none" : "none";
            try {
              const data = await fetchLabelTranslations();
            ...
            translationSupport.style.display =
              translationSupport.style.display === "none" ? "block" : "block";
          }
        }
      }
    }
...
function expandTKLabel(img) {
  ...
  let templateTextPerms = document.getElementById(
    "label-template-text-tk-perm"
  );
  let whyUseLabelTextPerms = document.getElementById(
    "why-use-this-label-tk-perm"
  );
...
function openLabelInfoDiv(targetDiv, divToCloseOne, divToCloseTwo) {
  ...
  if (targetDiv.classList.contains("hide")) {
    targetDiv.classList.replace("hide", "show");
  }

  // Close other two divs if open
  if (divToCloseOne.classList.contains("show")) {
    divToCloseOne.classList.replace("show", "hide");
  }
  if (divToCloseTwo.classList.contains("show")) {
    divToCloseTwo.classList.replace("show", "hide");
  }
}
...
function selectCommunities() {
  ...
  allOptionsMinusFirst.forEach((option) => {
    let selectedCommunityDiv = document.getElementById(
      `selected-community-${option.id}`
    );
...
function cloneForm(el) {
  ...
  if (window.location.href.includes("/projects/create-project")) {
    ...
    // Total forms hidden input needs to be incremented
    let hiddenInputs = document.getElementsByName(
      "additional_contributors-TOTAL_FORMS"
    );
    let totalFormInput = hiddenInputs[0];
...
if (
  ...
  addProjectLinkBtn.onclick = function () {
    ...
    for (const item of items) {
      ...
      if (isValidHttpUrl(item.trim())) {
        ...
        alert(
          "Please include full URL (with http or https) for project links."
        );
      }
    }
    input.value = "";
  ...
  function removeTargetDiv() {
    const removeProjectUrlBtns = document.querySelectorAll(
      ".removeProjectUrlBtn"
    );
  ...
  function selectContributors() {
    ...
    let contribOptionsArray = Array.from(
      document.getElementById("contributors").options
    );
    ...
    contribOptionsArray.forEach((option) => {
      ...
      if (option.value == contribInput.value) {
        ...
        if (contribInput.value.includes("Researcher")) {
          ...
          let selectedResearcherDiv = document.getElementById(
            `selected-researcher-${option.dataset.resid}`
          );
          ...
          let selectedInstitutionDiv = document.getElementById(
            `selected-institution-${option.dataset.instid}`
          );
          ...
          let selectedCommunityDiv = document.getElementById(
            `selected-community-${option.dataset.commid}`
          );
          selectedCommunityDiv.classList.replace("hide", "show");
  ...
  function removeSelectedContributors() {
    const removeResearcherBtns = document.querySelectorAll(
      ".removeSelectedResearcherBtn"
    );
    const removeInstitutionBtns = document.querySelectorAll(
      ".removeSelectedInstitutionBtn"
    );
    const removeCommunityBtns = document.querySelectorAll(
      ".removeSelectedCommunityBtn"
    );
    ...
    remove(
      removeResearcherBtns,
      "closeRes-",
      "selected-researcher-",
      "hiddenRes-"
    );
    remove(
      removeInstitutionBtns,
      "closeInst-",
...
function setProjectStatus(elem) {
  ...
  let statusSelectedInput = document.getElementById(
    `status-selection-input-${projectID}`
  );

  // Set second hidden value to value of option selected
  statusSelectedInput.value =
  ...
  if (checkboxes.length == 0) {
    ...
    if (!isChecked()) {
      const errorMsg = !isChecked()
        ? "At least one Notice must be selected."
        : "";
...
if (
  window.location.href.includes("connect-community") ||
  window.location.href.includes("connect-institution")
) {
...
function modalToggle(openBtnClasses, modalPartialId, closeBtnPartialId) {
  ...
  roleBtns.forEach((btn) => {
    ...
    const openChangeRoleBtn = document.getElementById(
      `${primary_id}_${user_id}`
    );
...
if (window.location.href.includes("members")) {
  ...
  document
    .getElementById("userListInput")
    .addEventListener("input", disableBtnDuringInput);
}
...
if (window.location.href.includes("manage")) {
  modalToggle(
    ".leaveCommunityBtn",
    "leaveCommAccountModal",
    "closeLeaveCommModal"
  );
  modalToggle(
    ".leaveInstitutionBtn",
    "leaveInstAccountModal",
    "closeLeaveInstModal"
  );
}
...
function disableBtnDuringInput() {
  ...
  document.getElementById("sendMemberInviteBtn").disabled =
...
if (deactivateAccountBtn) {
  deactivateAccountBtn.addEventListener("click", function (event) {
    ...
    cancelBtn.addEventListener("click", function (event) {
      deactivationModal.classList.replace("show", "hide");
    });

    let continueDeactivationBtn = document.getElementById(
      "continueDeactivationBtn"
    );
    continueDeactivationBtn.addEventListener("click", function () {
...
if (
  window.location.href.includes("communities/view/") ||
  window.location.href.includes("institutions/view/") ||
  window.location.href.includes("researchers/view/")
) {
  // Join request modal and form
  const openRequestToJoinModalBtn = document.getElementById(
    "openRequestToJoinModalBtn"
  );
  ...
  if (openRequestToJoinModalBtn) {
    openRequestToJoinModalBtn.addEventListener("click", function () {
      requestToJoinModal.classList.replace("hide", "show");
    });
  }
  ...
  function closeModal(modal) {
    let closeBtns = Array.from(
      document.getElementsByClassName("close-modal-btn")
    );
...
if (window.location.href.includes("dashboard")) {
  ...
  if (
    ...
    !localStorage.getItem("closedOnboarding")
  ) {
    // show modal
...
if (window.location.href.includes("notices")) {
  ...
  addURLBtn.addEventListener("click", () => {
    if (OTCModal.classList.contains("hide")) {
      OTCModal.classList.replace("hide", "show");
    }
  });
  const closeAddURLModal = document.getElementById("closeAddURLModal");
  closeAddURLModal.addEventListener("click", function () {
    OTCModal.classList.replace("show", "hide");
  });
  ...
  if (ccNoticeModal) {
    ...
    openCCModalBtn.addEventListener("click", (e) => {
      e.preventDefault();
      if (ccNoticeModal.classList.contains("hide")) {
        ccNoticeModal.classList.replace("hide", "show");
      }
    });
    const closeCCNoticeModal = document.getElementById("closeCCNoticeModal");
    closeCCNoticeModal.addEventListener("click", function () {
      ccNoticeModal.classList.replace("show", "hide");
    });
  }
}
...
if (
  ...
  !window.location.href.includes("/projects/embed/")
) {
  ...
  embedCode.value =
    '<iframe width="560" height="250" src="https://' +
    window.location.host +
    "/projects/embed/" +
    projectID +
    '/" title="Local Contexts Project Identifiers" frameborder="0"></iframe>';
  ...
  btnElem.addEventListener("click", function () {
    ...
    if (socialType == "email") {
      ...
      var emailBody = encodeURIComponent(
        "Check out this Local Contexts Project! " +
          projectTitle +
          " at " +
          projectURL
      );
      ...
      window.open(
        "http://www.facebook.com/sharer/sharer.php?u=" + projectURL,
        "Share on Facebook"
      );
      ...
      window.open(
        "https://twitter.com/intent/tweet?url=" +
          projectURL +
          "&text=Check out this @LocalContexts project! #LocalContexts #traditionalknowledge #indigenousdata",
        "Share on Twitter"
      );
...
if (window.location.href.includes("create-institution")) {
  ...
  const stateProvRegionInputField = document.getElementById(
    "institutionStateProvRegion"
  );
  ...
  nameInputField.addEventListener("input", () => {
    ...
    if (inputValue.length >= 3) {
      // Minimum characters required before making a request
  ...
  clearFormBtn.addEventListener("click", (e) => {
    ...
    if (
      nameInputField.getAttribute("readonly", true) &&
      nameInputField.classList.contains("readonly-input")
    ) {
...
if (
  window.location.href.includes("/institutions/update/") ||
  window.location.href.includes("/communities/update/") ||
  window.location.href.includes("/researchers/update/")
) {
  const realImageUploadBtn =
    document.getElementById("institutionImgUploadBtn") ||
    document.getElementById("communityImgUploadBtn") ||
    document.getElementById("researcherImgUploadBtn");
...
if (
  window.location.href.includes("/confirm-institution/") ||
  window.location.href.includes("/confirm-community/")
) {
  const realFileUploadBtn =
    document.getElementById("communitySupportLetterUploadBtn") ||
    document.getElementById("institutionSupportLetterUploadBtn");
  ...
  const contactEmailInput =
    document.getElementById("communityContactEmailField") ||
    document.getElementById("institutionContactEmailField");
  ...
  form.addEventListener("submit", function (e) {
    if (
      realFileUploadBtn.files.length === 0 &&
...
if (window.location.href.includes("institutions/notices/")) {
  ...
  const customFileUploadBtn = document.getElementById(
    "customCCPolicyFileUploadBtn"
  );
  ...
  function validatePolicyDocument() {
    ...
    if (file) {
      ...
      if (!allowedExtensions.includes(fileExt)) {
        ...
        customFileUploadBtn.innerHTML =
          'Upload Document <i class="fa-solid fa-upload"></i>';
        return false;
      }
  ...
  ccNoticeDownloadBtn.addEventListener("click", function () {
    ...
    ccNoticeDownloadBtn.innerHTML =
      'Downloading <div class="custom-loader margin-left-8"></div>';
...
if (
  window.location.href.includes("/communities/labels/customize/") ||
  window.location.href.includes("/communities/labels/edit/")
) {
  const realAudioUploadBtn = document.getElementById(
    "originalLabelAudioUploadBtn"
  );
  const customAudioUploadBtn = document.getElementById(
    "customLabelAudioFileUploadBtn"
  );
  const clearAudioFileBtn = document.getElementById(
    "clearAudiofileOnCustomizeBtn"
  );
  ...
  clearAudioFileBtn.addEventListener("click", function (e) {
    e.preventDefault();
    customAudioUploadBtn.innerHTML =
      'Upload audio file <i class="fa-solid fa-upload"></i>';
...
if (
  window.location.href.includes("communities/members/") ||
  window.location.href.includes("institutions/members/")
) {
  // Add member modal
  ...
  function closeMemberModal() {
    ...
    const elementClickedIsNotChild =
      !elementClickedIsChild || elementClickedIsSelf;
