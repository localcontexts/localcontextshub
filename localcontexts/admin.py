...
def dashboardData(end_date):
    ...
    bclabels_status = bclabels.aggregate(
        ...
        applied=Count(
            'id',
            filter=(Q(is_approved=True) & Q(project_bclabels__isnull=False)),
            distinct=True
        )
    )
    ...
    tklabels_status = tklabels.aggregate(
        ...
        applied=Count(
            'id',
            filter=(Q(is_approved=True) & Q(project_tklabels__isnull=False)),
            distinct=True
        )
    )
...
class InactiveAccountsAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        researcher_created_projects = ProjectCreator.objects.filter(
            researcher__isnull=False
        ).values_list('researcher_id')
